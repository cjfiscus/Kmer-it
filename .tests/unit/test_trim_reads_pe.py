...
def test_trim_reads_pe(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/trim_reads_pe/config")
        data_path = Path(".tests/unit/trim_reads_pe/data")
        expected_path = Path(".tests/unit/trim_reads_pe/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "results/pe/trim/b_1_trimmed.fq.gz",
                "results/pe/trim/b_1_trimmed_unpaired.fq.gz",
                "results/pe/trim/b_2_trimmed.fq.gz",
                "results/pe/trim/b_2_trimmed_unpaired.fq.gz",
                "--snakefile",
                "workflow/Snakefile",
                "-f",
                "--notemp",
