...
def test_ln_fastq_pe(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/ln_fastq_pe/config")
        data_path = Path(".tests/unit/ln_fastq_pe/data")
        expected_path = Path(".tests/unit/ln_fastq_pe/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "resources/pe/raw/b_1.fq.gz",
                "resources/pe/raw/b_2.fq.gz",
                "--snakefile",
                "workflow/Snakefile",
                "-f",
                "--notemp",
