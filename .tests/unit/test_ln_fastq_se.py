...
def test_ln_fastq_se(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/ln_fastq_se/config")
        data_path = Path(".tests/unit/ln_fastq_se/data")
        expected_path = Path(".tests/unit/ln_fastq_se/expected")
