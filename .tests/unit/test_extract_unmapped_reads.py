...
def test_extract_unmapped_reads(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/extract_unmapped_reads/config")
        data_path = Path(".tests/unit/extract_unmapped_reads/data")
        expected_path = Path(".tests/unit/extract_unmapped_reads/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "results/unmapped/b.unmapped.fq",
                "--snakefile",
                "workflow/Snakefile",
                "-f",
                "--notemp",
