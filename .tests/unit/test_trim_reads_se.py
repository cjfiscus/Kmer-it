...
def test_trim_reads_se(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/trim_reads_se/config")
        data_path = Path(".tests/unit/trim_reads_se/data")
        expected_path = Path(".tests/unit/trim_reads_se/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "results/se/trim/c_trimmed.fq.gz",
                "--snakefile",
                "workflow/Snakefile",
                "-f",
                "--notemp",
