...
def test_sam_2_sorted_bam_unmapped(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/sam_2_sorted_bam_unmapped/config")
        data_path = Path(".tests/unit/sam_2_sorted_bam_unmapped/data")
        expected_path = Path(".tests/unit/sam_2_sorted_bam_unmapped/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "results/unmapped/b.unmapped.bam",
                "--snakefile",
                "workflow/Snakefile",
                "-f",
                "--notemp",
